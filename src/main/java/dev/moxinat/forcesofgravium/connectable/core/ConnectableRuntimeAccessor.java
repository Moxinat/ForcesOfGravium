...
public final class ConnectableRuntimeAccessor {
    ...
    public static @Nonnull Map<Vector3i, ConnectableRuntimeData> snapshotForWorld(@Nonnull World world) {
        return ConnectableDataStore.snapshotForWorld(world);
    }
    ...
    public static @Nonnull Set<Vector3i> positionsMatching(@Nonnull World world, @Nonnull Predicate<String> blockIdPredicate) {
        LinkedHashSet<Vector3i> result = new LinkedHashSet<>();
        for (Vector3i position : snapshotForWorld(world).keySet()) {
            BlockType blockType = world.getBlockType(position.x(), position.y(), position.z());
            if (blockType != null && blockIdPredicate.test(blockType.getId())) {
                result.add(position);
            }
        }
    ...
    public static @Nonnull Set<Vector3i> carrierPositionsForWorld(@Nonnull World world) {
        return positionsMatching(world, ConnectableRegistry::isGravityPowderCarrierId);
    }

    public static @Nonnull Set<Vector3i> inverterPositionsForWorld(@Nonnull World world) {
        return positionsMatching(world, ConnectableRegistry::isInverterId);
    }
    ...
    public static @Nonnull RotationTuple getRotation(@Nonnull World world, @Nonnull Vector3i position) {
        ...
        ConnectableRuntimeData data = fromOldStores(world, position, blockType.getId(), RotationTuple.NONE, ConnectableRuntimeData.NO_NETWORK);
        ConnectableDataStore.put(world, position, data);
    ...
    public static @Nullable RotationTuple storedRotation(@Nonnull World world, @Nonnull Vector3i position) {
        ConnectableRuntimeData mirror = ConnectableDataStore.get(world, position);
        return mirror == null ? null : mirror.rotation();
    }
    ...
    public static @Nonnull String stateSuffixForSignalState(@Nullable String state) {
        return switch (GravityPowderSpecialStateStore.normalizeState(state)) {
            case GravityPowderSpecialStateStore.STATE_PUSH -> "Push";
            case GravityPowderSpecialStateStore.STATE_PULL -> "Pull";
            default -> "";
        };
    }
    ...
    private static @Nonnull ConnectableRuntimeData fromGravityPowder(
        ...
        GravityPowderBlockData data = GravityPowderSpecialStateStore.getOrCreate(world, position);
    ...
    private static @Nonnull ConnectableRuntimeData fromSource(
        ...
        return fromSource(world, position, rotation, energyDelta, networkId);
    }
    ...
    private static @Nonnull ConnectableRuntimeData fromSource(
        ...
        String state = energyDelta > 0 ? GravityPowderSpecialStateStore.STATE_PUSH : GravityPowderSpecialStateStore.STATE_OFF;
        return new ConnectableRuntimeData(
                rotation,
                state,
                state,
    ...
    private static boolean isGenericDefaultRuntime(@Nonnull ConnectableRuntimeData data) {
        return GravityPowderSpecialStateStore.STATE_OFF.equals(data.previousInstantState())
                && GravityPowderSpecialStateStore.STATE_OFF.equals(data.instantState())
                && GravityPowderSpecialStateStore.STATE_OFF.equals(data.previousEffectiveState())
                && GravityPowderSpecialStateStore.STATE_OFF.equals(data.effectiveState())
                && !data.dirty()
                && !data.invertEnabled()
                && !data.passing()
                && data.energyDelta() == 0;
    ...
    private static void writeGravityPowder(
        ...
        GravityPowderBlockData existing = GravityPowderSpecialStateStore.getOrCreate(world, position);
    ...
    private static void writeInverter(
        ...
        InverterData existing = InverterSpecialStateStore.getOrCreate(world, position);
